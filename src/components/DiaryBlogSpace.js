...
function DiaryBlogSpace({ isLoggedIn, setIsLoggedIn, selectedKey }) {
  ...
  const [selectedTypeitSpace, setSelectedTypeitSpace] = useState(null);
    ...
    const [blogSpacesData, setBlogSpacesData] = useState([]);
 ...
 useEffect(() => {
    const fetchCommentsCount = async () => {
      try {
        const response = await fetch(`https://typeit-api.onrender.com/blog_comments_count/${selectedTypeitSpace.$oid}`);
        const data = await response.json();
        setTotalCommentsCount(data.total_comments_count);
        console.log('Total Comments Count:', data.total_comments_count); // Add this line
    ...
    if (selectedTypeitSpace) {
      fetchCommentsCount();
    } else {
      // Reset comments count if selectedTypeitSpace is falsy
...
<div className="flex flex-wrap items-start justify-center">
  ...
  companyData
      ...
      >
        ...
        />
                          ...
                          <div className="flex-grow flex flex-col justify-between bg-white dark:bg-slate-800 p-6">
                            ...
                            <div className="flex space-x-2 text-sm dark:text-block-400 mt-2">
    ...
    <div className="flex flex-col items-center">
  <button type="button" className="flex items-center justify-center p-1 space-x-1.5">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" aria-label="Number of comments" className="w-4 h-4 fill-current text-violet-400 cursor-pointer hover:text-blue-500">
         <path
    ...
    {selectedTypeitSpace && (
      <>
        <CommentSection totalComments={totalCommentsCount} loading={loadingComments} />
        <span className="text-sm">{totalCommentsCount}</span>
      </>
    )}
  </button></div>
  <div className="flex flex-col items-center">
  <button aria-label="Edit post" type="button" className="flex items-center justify-center p-1 space-x-2">
